...
Or you can use git directly on server:  
If you previously made mfp directory, using this way of git installation, make sure to delete it previously  
$ rm -rf /home/mfp/mfp  
Note: rm -rf is dangerous command, can delete non-empty directories  
...
Yes. Installation dependencies are same. Update mfp_parameters.py with needed info. Also set dir_in in mfp.py and keep.py, in keep.py set jabber settings.  
...
Is project already started on server?  
...
If problem in xmpp messaging happen, problem may be eventually localised at your xmpp provider. Remember to previously mutually authorize xmpp accounts, and do not set active at home server xmpp account. keep.py will drive it.
